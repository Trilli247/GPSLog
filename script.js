...
function startApp() {
  const barleiter = document.getElementById("barleiter").value.trim();
  const barname = document.getElementById("barname").value.trim();
  const zeitpunkt = document.getElementById("zeitpunkt").value.trim();

  if (!barleiter || !barname || !zeitpunkt) {
    alert("Bitte alle Felder ausfüllen.");
    return;
  }

  userData = { barleiter, barname, zeitpunkt };
